...
feedwheat_values = sliced_df[sliced_df['category'].str.strip().str.lower() == 'feed_wheat']
print(feedwheat_values)
...
specific_column = feedwheat_values['index']

print(specific_column)

# Convert the list 'fbx_value' to a pandas Series
feed_wheat_series = pd.Series(specific_column)

# Calculate percentage change from the last value
latest_value_feed_wheat = feed_wheat_series.iloc[-1]
previous_month_value_feed_wheat = feed_wheat_series.iloc[-2]
last_year_value_feed_wheat = feed_wheat_series.iloc[-13]
monthly_change_feed_wheat = ((latest_value_feed_wheat - previous_month_value_feed_wheat) / previous_month_value_feed_wheat) * 100
annual_change_feed_wheat = ((latest_value_feed_wheat - last_year_value_feed_wheat) / last_year_value_feed_wheat) * 100

monthly_change_feed_wheat = round(monthly_change_feed_wheat,1)
annual_change_feed_wheat = round(annual_change_feed_wheat,1)

# Print or use the calculated changes as needed
print("Latest value for Feed Wheat:", feed_wheat_series.iloc[-1])
print("Monthly Change Feed Wheat:", monthly_change_feed_wheat)
print("Annual Change Feed Wheat:", annual_change_feed_wheat)
...
plt.plot(specific_x_values,specific_column, label='Feed wheat (2020=100)', color='#3E71B3')

plt.ylabel('Index Value', color='#797878', fontsize=8)
plt.title('Feed wheat', color='#001E60', fontsize=12)
