...
columns_to_read = ["World Bank Commodity Price Data (The Pink Sheet)", "Unnamed: 47"] # Replace with your column names
...
sugar_series = pd.Series(specific_rows['Unnamed: 47'])

# Calculate percentage change from the last value
latest_value_sugar = sugar_series.iloc[-1]
previous_month_value_sugar = sugar_series.iloc[-2]
last_year_value_sugar = sugar_series.iloc[-13]
monthly_change_sugar = ((latest_value_sugar - previous_month_value_sugar) / previous_month_value_sugar) * 100
annual_change_sugar= ((latest_value_sugar - last_year_value_sugar) / last_year_value_sugar) * 100

monthly_change_sugar = round(monthly_change_sugar,1)
annual_change_sugar = round(annual_change_sugar,1)

# Print or use the calculated changes as needed
print("Latest value for Sugar:", sugar_series.iloc[-1])
print("Monthly Change Sugar:", monthly_change_sugar)
print("Annual Change Sugar:", annual_change_sugar)
...
plt.plot(specific_x_values,specific_rows['Unnamed: 47'], label='Sugar ($/kg)', color='#3E71B3')

plt.ylabel('Price, $', color='#797878', fontsize=8)
plt.title('Sugar', color='#001E60', fontsize=12)
